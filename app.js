...
function renderScenarioManager() {
  ...
  if (!scenarios.length) {
    list.innerHTML = '<div style="color:var(--text3);font-size:12px;padding:8px">Keine Szenarien vorhanden</div>';
    return;
  }
...
document.getElementById('btn-add-scenario').addEventListener('click', () => {
  ...
  if (scenarios.some(s => s.abbr === abbr)) { showToast('⚠ Abkürzung bereits vorhanden'); return; }
